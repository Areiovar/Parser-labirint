...
async def get_page_data(session, page):
    ...
    link = f"https://www.labirint.ru/search/программирование/?stype=0&display=table&page={page}"

    async with session.get(url=link, headers=headers) as response:
        requests_text = await response.text()
...
async def gather_data():
    ...
    link = "https://www.labirint.ru/search/программирование/?stype=0&display=table"
...
def main():
    ...
    with open(f"labirint_{current_time}_async.json", "w") as file:
        json.dump(books_data_list, file, indent=4, ensure_ascii=False)

    with open(f"labirint_{current_time}_async.csv", "w") as file:
    ...
    for book in books_data_list:
        ...
        with open(f"labirint_{current_time}_async.csv", "a") as file:
            writer = csv.writer(file)
    ...
    finish_time = time.time() - start_time_prog
